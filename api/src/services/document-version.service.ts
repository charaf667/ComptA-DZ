...
class DocumentVersionService {
   ...
   */
  private readData(): Record<string, DocumentVersion[]> {
    try {
   ...
   */
  private writeData(data: Record<string, DocumentVersion[]>): void {
    try {
  ...
  async restoreVersion(
    ...
    restoredBy: string, 
    comment: string = `Restauration de la version ${versionNumber}`
    ...
    const newVersion: CreateVersionRequest = {
      documentId,
      comment,
