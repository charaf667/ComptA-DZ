...
const adaptive_learning_service_1 = __importDefault(require("./adaptive-learning.service"));
class AiClassificationService {
     ...
     * @returns Liste de suggestions de comptes
     */
    suggestAccountByFournisseur(fournisseur) {
        if (!fournisseur)
            return [];
        const suggestions = [];
        ...
        for (const rule of this.fournisseurRules) {
            if (fournisseurLower.includes(rule.nom.toLowerCase())) {
                ...
                switch (rule.priorite) {
                    case 1:
                        scoreConfiance = 0.98;
                        break; // Score plus élevé pour les fournisseurs
                    case 2:
                        scoreConfiance = 0.90;
                        break;
     ...
     * @returns Liste de suggestions de comptes
     */
    suggestAccountByReference(reference) {
        if (!reference)
            return [];
        const suggestions = [];
     ...
     */
    classifyDocument(extractedData) {
        let allSuggestions = [];
        ...
        try {
            const adaptiveSuggestions = adaptive_learning_service_1.default.suggestAccounts(extractedData);
     ...
     */
    recordUserFeedback(extractedData, selectedAccount) {
        try {
            // Enregistrer le feedback dans le service d'apprentissage adaptatif
            adaptive_learning_service_1.default.recordFeedback(extractedData, selectedAccount);
    ...
    generateJournalEntry(data, suggestion) {
        if (!data.montant || !suggestion)
            return null;
...
exports.default = new AiClassificationService();
